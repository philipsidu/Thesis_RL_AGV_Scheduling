...
def export_statistics_logging(statistics, parameters, resources):
    ...
    for mach in range(parameters['NUM_MACHINES']):
        ...
        for stat in list_of_stats:
            if stat == 'stat_machines_working':
                ...
                if resources['machines'][mach].last_process_start + resources['machines'][mach].last_process_time > \
                        statistics['time_end']:
                    statistics[stat][mach] -= resources['machines'][mach].last_process_start + resources['machines'][
                        mach].last_process_time - statistics['time_end']
            if stat == 'stat_machines_broken':
                if resources['machines'][mach].last_broken_start + resources['machines'][mach].last_broken_time > \
                        statistics['time_end']:
                    statistics[stat][mach] -= resources['machines'][mach].last_broken_start + resources['machines'][
                        mach].last_broken_time - statistics['time_end']

    statistics['stat_machines_working'] = np.true_divide(statistics['stat_machines_working'], statistics['time_end'])
    statistics['stat_machines_changeover'] = np.true_divide(statistics['stat_machines_changeover'],
                                                            statistics['time_end'])
    ...
    for i in range(parameters['NUM_MACHINES']):
        ce.printLog("{0:.3f}".format(statistics['stat_machines_working'][i]),
                    "{0:.3f}".format(statistics['stat_machines_changeover'][i]),
                    "{0:.3f}".format(statistics['stat_machines_broken'][i]),
                    "{0:.3f}".format(statistics['stat_machines_idle'][i]), " || ",
                    "{0:.3f}".format(
                        statistics['stat_machines_working'][i] + statistics['stat_machines_changeover'][i] +
                        statistics['stat_machines_broken'][i] + statistics['stat_machines_idle'][i]))
    ce.printLog("--------------------------")
    ce.printLog("{0:.3f}".format(np.mean(statistics['stat_machines_working'])),
                "{0:.3f}".format(np.mean(statistics['stat_machines_changeover'])),
                "{0:.3f}".format(np.mean(statistics['stat_machines_broken'])),
                "{0:.3f}".format(np.mean(statistics['stat_machines_idle'])), " || ",
                "{0:.3f}".format(np.mean(statistics['stat_machines_working']) + np.mean(
                    statistics['stat_machines_changeover']) + np.mean(statistics['stat_machines_broken']) + np.mean(
    ...
    for i in range(parameters['NUM_TRANSP_AGENTS']):
        ce.printLog("{0:.3f}".format(statistics['stat_transp_working'][i]),
                    "{0:.3f}".format(statistics['stat_transp_walking'][i]),
                    "{0:.3f}".format(statistics['stat_transp_handling'][i]),
                    "{0:.3f}".format(statistics['stat_transp_idle'][i]), " || ",
                    "{0:.3f}".format(statistics['stat_transp_walking'][i] + statistics['stat_transp_handling'][i] +
                                     statistics['stat_transp_idle'][i]))
    ce.printLog("--------------------------")
    ce.printLog("{0:.3f}".format(np.mean(statistics['stat_transp_working'])),
                "{0:.3f}".format(np.mean(statistics['stat_transp_walking'])),
                "{0:.3f}".format(np.mean(statistics['stat_transp_handling'])),
                "{0:.3f}".format(np.mean(statistics['stat_transp_idle'])), " || ",
                "{0:.3f}".format(
                    np.mean(statistics['stat_transp_walking']) + np.mean(statistics['stat_transp_handling']) + np.mean(
                        statistics['stat_transp_idle'])))
    ce.printLog("##########################")
    ...
    for kpi in pd_episode_log.columns:
        if kpi in ['dt', 'dt_real_time', 'valid_actions', 'total_reward', 'machines_total', 'selected_idle',
                   'forced_idle', 'threshold_waiting', 'finished_orders', 'processed_orders']:
            lq_stats.update({kpi: np.average(pd_episode_log[kpi].tail(last_quarter).tolist())})
        elif kpi in ['machines_working', 'machines_changeover', 'machines_broken', 'machines_idle',
                     'machines_processed_orders', 'transp_working', 'transp_walking', 'transp_handling', 'transp_idle',
                     'alpha', 'inventory']:
            ...
            lq_stats.update(
                {kpi: np.average(pd_episode_log[kpi].tail(last_quarter).tolist(), weights=dt_weights_orders)})
        else:
