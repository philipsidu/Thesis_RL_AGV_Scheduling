...
class TimeCalc:
    def __init__(self, parameters, episode):
        """
        Random numbers generated and stored in randomStreams dict
        ...
        self.randomStreams = {"process_time": [np.random.RandomState(np.random.randint(100)) for i in
                              ...
                              "machine_failure": [np.random.RandomState(np.random.randint(100)) for i in
                                                  range(parameters['NUM_MACHINES'])],
                              "repair_time": [np.random.RandomState(np.random.randint(100)) for i in
                                              range(parameters['NUM_MACHINES'])],
    ...
    @staticmethod
    def get_inventory_level(statistics):
        """
    ...
    def processing_time(self, machine, statistics, parameters, order):
        ...
        :param machine: machine Resource
        ...
        result_time = min(parameters['MAX_PROCESS_TIME'][machine.id],
                          max(parameters['MIN_PROCESS_TIME'][machine.id],
                              self.randomStreams["process_time"][machine.id].exponential(
                                  scale=parameters['AVERAGE_PROCESS_TIME'][machine.id])))
        statistics['stat_machines_working'][machine.id] += result_time
        return result_time

    @staticmethod
    def transp_time(start, end, transp, statistics, parameters):
        """
    ...
    @staticmethod
    def handling_time(machineorsource, loadorunload, transp, statistics, parameters):
        """
        Return actual handling time for a concrete part.
        :param machineorsource:
        :param loadorunload:
        :param transp:
        :param statistics:
        :param parameters:
    ...
    @staticmethod
    def changeover_time(machine, current_variant, next_variant, statistics, parameters):
        """
        Return actual changeover time for a machine.
        :param machine:
        :param current_variant:
        :param next_variant:
        :param statistics:
        :param parameters:
        :return result_time:
    ...
    def repair_time(self, machine, statistics, parameters):
        """
        Return repair time for a machine.
        :param machine:
        :param statistics:
        :param parameters:
        :return result_time:
    ...
    def time_to_order_generation(self, source, statistics, parameters):
        """
        Return time until next order is generated.
        :param source:
        :param statistics:
        :param parameters:
        :return result_time:
...
def update_exp_weights_std(**kwargs):
    """
...
class ZScoreNormalization(object):
    ...
    def __call__(self, value):
        """
        By calling this function calculates the mean, std and z score normalization of the next iteration with given
        value.
        :param value:
        :return:
        """
