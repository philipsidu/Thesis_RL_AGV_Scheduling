...
from logger import *
from prod.RLenv.init_env import *
from prod.RLenv.resources import *
from prod.RLenv.time_calc import TimeCalc
...
class ProductionEnv(Environment):
    ...
    def __init__(self, **kwargs):
        ...
        self.time_calc = TimeCalc(parameters=self.parameters, episode=self.count_episode)
    ...
    def execute(self, actions):
        ...
        if (self.counter % self.parameters[
            'EXPORT_FREQUENCY'] == 0 or self.counter % self.max_episode_timesteps() == 0) and not self.parameters[
            'EXPORT_NO_LOGS']:
        ...
        for agent in Transport.agents_waiting_for_action:
            ...
            # Waiting until action is processed in simulation environment
            ...
            self.statistics['stat_agent_reward'].append(
                [self.count_episode, self.counter, round(self.env.now, 5), None, None, None, states])
    ...
    def actions(self):
        ...
        """
        if self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'direct':
            # print("Actions: ", self.resources['transps'][0].mapping)
            number = len(self.resources['transps'][0].mapping)
        elif self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'resource':
            shape = (2,)
            number = len(self.resources['transps'][0].mapping)
