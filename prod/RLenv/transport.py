...
from prod.RLenv.reward_functions import *


class Transport(Resource):
    # Overall list of available transport orders
    all_transp_orders = []
    agents_waiting_for_action = []

    def __init__(self, env, id, resp_area, agent_type, statistics, parameters, resources, agents, time_calc,
                 location, label):
        ...
        self.type = "transp"
        self.idle = env.event()
        self.current_location = self.time_calc.randomStreams["transp_agent"][self.id].choice(self.resources["sources"])
        ...
        if self.agent_type == "FIFO":
            self.agent = DecisionHeuristicTranspFIFO(env=self.env, statistics=statistics, parameters=parameters,
                                                     resources=resources, agents=agents, agents_resource=self)
        elif self.agent_type == "NJF":
            self.agent = DecisionHeuristicTranspNJF(env=self.env, statistics=statistics, parameters=parameters,
                                                    resources=resources, agents=agents, agents_resource=self)
        elif self.agent_type == "EMPTY":
            self.agent = DecisionHeuristicTranspEMPTY(env=self.env, statistics=statistics, parameters=parameters,
                                                      resources=resources, agents=agents, agents_resource=self)
    ...
    def get_next_action(self):
        ...
        for order in [x for x in Transport.all_transp_orders
                      if x.get_total_waiting_time() > self.parameters['WAITING_TIME_THRESHOLD'] and not x.reserved]:

            if order.get_next_step().type == 'machine':
                for destination in [x for x in self.resources['machines']
                                    if x.machine_group == order.get_next_step().machine_group]:
                    result_order, result_origin, result_destination = self.get_order_destination(
                        order=order, origin=order.current_location, destination=destination)
            else:
                result_order, result_origin, result_destination = self.get_order_destination(
                    order=order, origin=order.current_location, destination=order.get_next_step())
        ...
        if result_order is None:  # Only for invalid actions
    ...
    def calculate_state(self):
        ...
        if 'total_process_time' in self.parameters['TRANSP_AGENT_STATE']:
            ...
            state = [0.0] * self.parameters['NUM_MACHINES']
            for mach in self.resources['machines']:
                if mach.buffer_processing is not None:
                    state[mach.id] = mach.last_process_time - (self.env.now - mach.last_process_start)
    ...
    def calculate_reward(self, action):
        ...
        # If true, then invalid action selected
    ...
    def transporting(self):
        ...
        while True:
            ...
            if self.parameters['PRINT_CONSOLE']:
                ...
                print("Machine state")
                print("Legende:  MachineID :  Inbound-Buffer Orders  -  Idle? True/False // Inside Machine Order  -  "
                      "Outbound-Buffer Orders  -  Broken? True/False")
                for machine in self.resources['machines']:
                    print("Machine", machine.id, ": ", len(machine.buffer_in), [x.id for x in machine.buffer_in], " - ",
                          ...
                          [machine.buffer_processing.id if machine.buffer_processing is not None else " "], " - ",
            ...
            if self.parameters['PRINT_CONSOLE']:
                ...
                for ord in Transport.all_transp_orders:
                    print("Order", ord.id, ":  ", ord.current_location.id, " -> ", ord.get_next_step().id,
                          ...
                          "***" if order is not None and order != -1 and order.id == ord.id else "")  # Order that is selected is highlighted by ***

            if order == -1 and destination == -1:  # If waiting action
                if self.parameters['PRINT_CONSOLE']:
                    print("Waiting action selected. Idle for ",
                          self.parameters['TRANSP_AGENT_WAITING_TIME_ACTION'], " time units.")
            ...
            elif order == -1 and destination != -1 and destination is not None:  # If empty action
            ...
            if order is None:  # If invalid action
